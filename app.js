...
function fotoGekozen(e) {
  ...
  reader.onload = ev => {
    ...
    img.onload = () => {
      ...
      huidigeFoto = canvas.toDataURL('image/jpeg', 0.5);
...
function verstuurMelding() {
  ...
  if (navigator.share) {
    navigator.share({ title: 'SpoorNet', text: deelTekst, url }).catch(() => {});
  } else if (navigator.clipboard) {
    navigator.clipboard.writeText(url).then(() => toon(TR[lang].toast_gekopieerd_link));
  }
  resetMeldForm();
}

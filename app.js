...
const TR = {
  nl: {
    ...
    role_sub:'Je kunt dit op elk moment weer wijzigen via het 🔁-knopje rechtsboven — handig als je halverwege de dag van rol wisselt met iemand anders in de groep.',
    ...
    anders_label:'Beschrijving',
    huisdier_label:'Welk huisdier?',
    huisdier_placeholder:"Bv. Witte poes 'Miep'",
  ...
  it: {
    ...
    role_sub:'Puoi sempre cambiare in seguito tramite il pulsante 🔁 in alto a destra — utile se durante la giornata ti scambi il ruolo con qualcun altro del gruppo.',
    ...
    anders_label:'Descrizione',
    huisdier_label:'Quale animale?',
    huisdier_placeholder:"Es. Gatta bianca 'Miep'",
...
function gaNaar(view) {
  ...
  ['role', 'melden', 'kaart', 'info'].forEach(v => {
...
function kiesCategorie(id) {
  huidigeCat = id;
  renderCatGrid();
  const toonVeld = (id === 'anders' || id === 'huisdier');
  document.getElementById('anders-field').classList.toggle('hidden', !toonVeld);
  if (toonVeld) {
    const labelKey = id === 'huisdier' ? 'huisdier_label' : 'anders_label';
    const placeholderKey = id === 'huisdier' ? 'huisdier_placeholder' : 'anders_placeholder';
    document.getElementById('anders-label').textContent = TR[lang][labelKey];
    document.getElementById('anders-tekst').placeholder = TR[lang][placeholderKey];
  }
  updateVerzendKnop();
}
...
function verstuurMelding() {
  ...
  const melding = {
    ...
    cat: huidigeCat,
    tekst: (huidigeCat === 'anders' || huidigeCat === 'huisdier') && andersTekst ? andersTekst.value.trim() : '',
...
function tekenKaart() {
  ...
  lijst.forEach(m => {
    ...
    const naam = (m.cat === 'anders' || m.cat === 'huisdier') && m.tekst ? esc(m.tekst) : catLabel(cat);
...
function kopieerSamenvatting() {
  ...
  const regels = lijst.map(m => {
    const cat = CATS.find(c => c.id === m.cat) || CATS[3];
    const naam = (m.cat === 'anders' || m.cat === 'huisdier') && m.tekst ? `${catLabel(cat)}: ${m.tekst}` : catLabel(cat);
    const tijd = new Date(m.tijd).toLocaleString(lang === 'it' ? 'it-IT' : 'nl-NL');
